...
def trainModel(data, rank=4, iters=4, reg=0.35, fullData=True, justTrain=True) -> ALSModel:
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id, run_name='spark_als_model'):
        ...
        # Save model to S3 directly — in cluster mode, executors write
        # parquet to their own local filesystems, so mlflow.spark.log_model
        # only captures _SUCCESS markers without actual data files.
        # The model server loads from this S3 path instead.
        s3_model_path = "s3a://mlflow/models/restaurant_recommender"
        model.write().overwrite().save(s3_model_path)
        mlflow.log_param("model_s3_path", s3_model_path)
    return model
